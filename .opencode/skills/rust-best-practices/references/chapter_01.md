...
let user = String::from("foo");
process_string(user.clone()); // Unnecessary clone
```
...
assert_eq!(x.ok_or(ParseError::Missing), Err(ParseError::Missing));

let missing = "value";
let x: Option<i32> = None;
assert_eq!(
    x.ok_or_else(|| ParseError::ValueAbsent(format!("missing {missing}"))),
    Err(ParseError::ValueAbsent("missing value".to_string()))
);
...
let x: Result<&str, String> = Ok("foo");
assert_eq!(x.map_or_else(|e| e.len(), |v| v.len()), 3);

let x: Option<Vec<i32>> = None;
assert_eq!(x.unwrap_or_else(Vec::new), Vec::new());
```
...
```rust
let x: Result<&str, String> = Ok("foo");
// Always allocates the error string, even on the `Ok` path:
assert_eq!(x.map_or(format!("uninformed error"), |v| v.to_string()), "foo");

let x: Option<Vec<i32>> = None;
assert_eq!(x.unwrap_or(Vec::new()), Vec::new()); // prefer `.unwrap_or_default()`

let missing = "value";
let x: Option<i32> = None;
// Eagerly builds the error even when `x` is `Some`:
assert_eq!(
    x.ok_or(ParseError::ValueAbsent(format!("missing {missing}"))),
    Err(ParseError::ValueAbsent("missing value".to_string()))
);
```
...
- Prefer `iter` over `into_iter` unless you need to consume the collection and take ownership of its elements.
- When you only need to read `Copy` values without consuming the collection, use `.iter().copied()` (e.g. on a `Vec<i32>`) rather than cloning or moving it.
...
> Prefer to express the intent in the function's name, or split it into smaller functions.
...
> More on keeping comments honest in [Chapter 8 - Comments](./chapter_08.md)
