...
export function createClient(apiKey: string) {
  ...
  return {
    async getPlayerProfile(platform: string, username: string) {
      ...
      try {
        [accountInfo, statsRanked, operatorStats] = await Promise.all([
          r6.players.getAccountInfo({ nameOnPlatform: username, platformType }).catch(() => null),
          r6.players
            .getPlayerStats({
            ...
            .catch(() => null),
          r6.players
            .getOperatorStats({ nameOnPlatform: username, platformType, modes: "ranked" })
            .catch(() => null),
        ]);
      } catch (err) {
